...
export abstract class CoinbaseService {
  ...
  private static getHttpHeaders(signature: string, timestamp: number) {
    return new HttpHeaders()
      .append('CB-ACCESS-KEY', CoinbaseService.API_KEY)
  ...
  static requete(httpMethod: string, requestPath: string, payload: string, http: HttpClient) {
    ...
    return http.get(this.BASE_URL + requestPath, { headers })
  }
